...
st.write("<h5 style='color: grey;font-style: italic;'>Estimate carbon emission amount for your data centers and get personalized recommendations</h5>", unsafe_allow_html=True)
...
with col1:
    st.write("<h3 style='color: green;font-style: italic;'>Server Energy and Carbon Data</h3>", unsafe_allow_html=True)
    server_inventory = st.radio(questions["Server Energy and Carbon"][0], ["Yes", "No"])
    num_servers = st.number_input(questions["Server Energy and Carbon"][1], min_value=0, step=1)
    avg_cpus = st.number_input(questions["Server Energy and Carbon"][2], min_value=0, step=1)
    memory_input = st.number_input(questions["Server Energy and Carbon"][3], min_value=0, max_value=55000, step=100)
    memory_type = st.selectbox(questions["Server Energy and Carbon"][4], ["SSD", "HDD"])

# Display questions and gather inputs for "PUE Model"
with col2:
    st.write("<h3 style='color: green;font-style: italic;'>PUE Model Input Data</h3>", unsafe_allow_html=True)
    ...
    cooling_system = st.selectbox(questions["PUE Model"][7], ["Air-Cooled DX", "Water-Cooled DX", "Evaporatively-Cooled DX", "Chilled Water"])
    chiller_type = st.selectbox(questions["PUE Model"][8], ["air-cooled", "water-cooled"])
    chilled_water_temp = st.number_input(questions["PUE Model"][9], min_value=-50, step=1)
    ups_type = st.selectbox(questions["PUE Model"][10], ["Double Conversion", "Double Conversion + Filter", "Delta Conversion", "Rotary", "None"])
...
if st.button("File Upload"):
    ...
    uploaded_file = st.file_uploader("Choose a file", type=["csv", "txt", "xlsx"], label_visibility="collapsed")
...
st.write("<h3 style='color: green;font-style: italic;'>Estimate Carbon Footprint</h3>", unsafe_allow_html=True)
...
if st.button("Predict Carbon Emission"):
    ...
    st.write(f"#### Predicted Carbon Emission (by XGBoost): {carbon_emission_pred_xgb[0]:.2f} units")
...
st.write("<h3 style='color: green;font-style: italic;'>Get Your Personalized Recommendations</h3>", unsafe_allow_html=True)

# Output Recommendation
if st.button("Get Recommendations"):
    
    # Display the recommendations
    st.write(f"#### we recommend...")
